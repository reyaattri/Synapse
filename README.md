...
- **Autonomous signal discovery, not just lookup.** `/discover_signals` doesn't check a pair a user already suspects — it walks every drug pair that actually co-occurs across the fleet's isolated datasets and flags any pair *not already known to Synapse* where a symptom appears disproportionately more often among exposed patients than unexposed ones, using the same textbook Proportional Reporting Ratio formula regulators use to screen adverse-event databases. This is the difference persistent memory is supposed to make: the system doesn't just answer questions about interactions someone already knew to ask about, it can surface ones nobody told it existed, and it gets more capable of that the more patients it remembers. Every patient's raw notes are still only ever read from that patient's own isolated dataset — the sweep aggregates already-isolated per-patient summaries, never cross-patient text.
...
- **Proportional Reporting Ratio** (Evans, Waller & Davis, 2001) — the disproportionality statistic regulators use to screen spontaneous adverse-event reports (FDA FAERS, WHO VigiBase) for signals worth investigating. Synapse computes this two ways: `/discover_signals` uses the textbook 2×2 contingency formula on real per-patient exposure/outcome data (patients with both drugs active vs. not, a given symptom present vs. not), the same shape as the original formula. `/ledger/{finding_title}` uses an adapted version over the clinician feedback ledger instead — how disproportionately often *this* finding is confirmed vs. dismissed relative to every other finding the fleet has judged — since there's no independent adverse-event denominator for feedback events. Both use the conventional FAERS screening rule (PRR ≥ 2, at least a handful of reports) to flag a signal.
